...
// import { useCopyToClipboard } from '@/hooks/use-copy-to-clipboard'; // 暂时不引入，避免引入新文件
// import { generateId } from 'ai'; // 暂时不引入，避免引入新文件
...
const CodeBlock: FC<Props> = memo(({ language, value }) => {
  // const { isCopied, copyToClipboard } = useCopyToClipboard({ timeout: 2000 }); // 暂时不使用
  ...
  const downloadAsFile = () => {
    ...
    // const suggestedFileName = `file-${generateId()}${fileExtension}`; // 暂时不使用
